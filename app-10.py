...
<div class="model-row"><span>当前模型：</span><span class="model-name" id="modelName">__PRETTY_MODEL__</span><span class="pill">按次计费</span><span class="api-source" id="apiSource">__API_SOURCE__</span><button class="detail-link" type="button" id="apiBtn">查看详情 ›</button></div>
</section>__MSG__
...
const STORE='gongkan_persona_v19', THEME_STORE='gongkan_theme_v19', API_STORE='gongkan_api_v19', FAV_STORE='gongkan_favorites_v20';
const THEME_META={matcha:'#f8f4e9',rose:'#faf0f1',mist:'#eef4f6',apricot:'#fbf1e6',lilac:'#f3f0f8'};
let cfg=loadCfg(), apiCfg=loadApiCfg(), lastError='', lastRecent=[];
...
$('navHome').onclick=()=>{setNav('navHome');window.scrollTo({top:0,behavior:'smooth'})};$('navRecords').onclick=()=>{setNav('navRecords');$('recentSection').scrollIntoView({behavior:'smooth',block:'start'})};$('navMine').onclick=()=>{setNav('navMine');openSettings()};
function renderFavorites(){const favs=getFavs();const rows=lastRecent.filter(r=>favs.has(String(r.video_id)));$('favoriteList').innerHTML=rows.length?rows.map(r=>`<a class="favorite-row" href="/report/${encodeURIComponent(r.video_id)}"><strong>★ ${esc(shortName(r.original_name))}</strong><small>${esc(r.summary||'点击查看完整报告')}</small></a>`).join(''):`<div class="favorite-empty">还没有收藏。分析出记录后，点记录旁边的 ☆ 就能收进这里。</div>`}
$('navFavorites').onclick=async()=>{setNav('navFavorites');if(!lastRecent.length)await refreshRecent();renderFavorites();openModal('favoritesModal')};$('closeFavorites').onclick=()=>{closeModal('favoritesModal');setNav('navHome')};$('favoritesModal').addEventListener('click',e=>{if(e.target===$('favoritesModal')){$('closeFavorites').click()}});
applyPersona();updateApiBadge();refreshRecent();
</script></body></html>"""
...
@app.get("/health")
def health():
    return {"ok": True, "name": "共看", "ui": "v2.0-public-full"}
