...
.shell{width:min(100%,760px);margin:0 auto;padding:22px 16px calc(44px + env(safe-area-inset-bottom));position:relative;overflow:hidden}
.decor{position:absolute;right:-6px;top:22px;width:138px;height:150px;opacity:.42;pointer-events:none}
.hero{position:relative;display:grid;grid-template-columns:92px minmax(0,1fr);gap:22px;align-items:center;padding:18px 4px 22px;min-width:0;z-index:1}
.hero>div{min-width:0}.logo{width:88px;height:88px;border-radius:26px;background:#fffaf0;object-fit:cover;box-shadow:0 14px 30px rgba(78,96,69,.15);border:1px solid rgba(220,210,188,.8)}
.hero h1{margin:0 0 9px;color:var(--ink);font-size:clamp(42px,11vw,62px);line-height:.98;letter-spacing:.05em;font-weight:850}
.hero p{margin:0;max-width:460px;color:#506057;font-size:17px;line-height:1.75}
...
@media(max-width:560px){.shell{padding:16px 13px 36px}.decor{width:116px;height:130px;right:-20px;top:14px}.hero{grid-template-columns:76px minmax(0,1fr);gap:14px;padding-top:16px}.logo{width:74px;height:74px;border-radius:22px}.hero h1{font-size:43px}.hero p{font-size:15px;line-height:1.65;padding-right:24px}.card{padding:18px;border-radius:25px}.status{padding:18px}.status-top{grid-template-columns:minmax(0,1fr) minmax(0,1fr) auto;gap:10px}.status-item{font-size:14px;gap:7px}.status-icon{width:30px;height:30px;flex-basis:30px}.lock-icon{width:30px;height:30px}.upload{padding:22px 18px}.file-row{gap:10px}.file-label{padding:11px 13px;font-size:14px}.filename{font-size:13px}.primary{font-size:20px;min-height:58px}.section-title h2{font-size:22px}}
...
<section class="card status"><div class="status-top">
  <div class="status-item"><span class="status-icon">{model_dot}</span><span>视频理解</span></div>
  <div class="status-item"><span class="status-icon{password_cls}">{password_dot}</span><span>网页保护</span></div>
...
</div><div class="model">接口模式：{html.escape(api_mode)}　·　主模型：{html.escape(model)}</div></section>{msg}
<section class="card upload"><form action="/upload" method="post" enctype="multipart/form-data">
  <div class="file-row"><label class="file-label"><span>▣</span><span>选择视频</span><input id="videoInput" type="file" name="video" accept="video/*" required></label><span id="fileName" class="filename">未选择任何文件</span></div>
...
<div class="footnote"><span class="info">i</span><div>建议 60 秒以内，单个视频不超过 {MAX_UPLOAD_MB}MB。服务器会在需要时自动转换成适合模型读取的格式；原视频分析完成后删除。</div></div>
